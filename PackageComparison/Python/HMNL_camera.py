...
# Forces Python to start looking from the 'Python' folder, not 'liesel_project'
json_path = os.path.join(script_location, "..", "Data", "camera_data.json")

# Load the data
data_dict = load_camera_data(json_path)

k_dim = int(data_dict["n_params"]) # Should be 10 for camera data
...
# Duration: Adjusted for real data (current setup seems sufficient and saves runtime)
...
liesel_data_path = os.path.join(script_location, "..", "Data", "liesel_output_camera.pkl")
