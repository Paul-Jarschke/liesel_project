...
def load_margarine_data(filepath):
    ...
    # Post-process Demographics
    Z_arr = np.array(Z_list)
    Z_arr = np.log(Z_arr)
    Z_arr = Z_arr - np.mean(Z_arr, axis=0)
    
    # NEW: Add intercept column to Z so the first row of Delta acts as the baseline mu
...
# Note: 'mu' kernel is removed, replaced entirely by 'Delta'
...
n_chains_actual = samples['Delta'].shape[0]
n_iter_actual = samples['Delta'].shape[1]
...
filename = f"liesel_output_margarine_paper_redo_{total_samples_count}_samples.pkl"
