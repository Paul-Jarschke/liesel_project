...
def load_margarine_data(filepath):
    ...
    # Margarine data specifics
    n_alts = 10  # 10 Products
    
    for i, respondent in enumerate(raw_data):
        X_raw = np.array(respondent['X'])
        y_raw = np.array(respondent['y'])
        
        # --- FIX: Handle scalar y caused by JSON unboxing ---
        # If a household has 1 obs, y might be a 0-d array (scalar)
        if y_raw.ndim == 0:
        ...
        # The JSON X is stacked (Obs*Alts, Params)
        X_reshaped = X_raw.reshape((n_choice_occasions, n_alts, n_params))
        
        # ADJUST Y: 0-based indexing
        y_adjusted = y_raw - 1
...
filename = f"liesel_output_margarine_{total_samples_count}_samples.pkl"
